...
const CSS = `
  ...
  .marca-row.open { border-color:var(--blue2); }
  ...
  .marca-header:hover { background:var(--field-bg); }
  ...
  .modelo-item:hover { background:var(--field-bg); }
...
function ImgCarousel({ fotos, title }: { fotos:string[]; title:string }) {
  ...
  if (!fotos.length) return (
    <div style={{height:280,background:'linear-gradient(135deg,var(--border),var(--field-bg))',borderRadius:16,display:'flex',alignItems:'center',justifyContent:'center',fontSize:60,opacity:.4}}>📱</div>
  )
  return (
    <div className="img-carousel">
      ...
      {fotos.length > 1 && (
        <>
          <button onClick={()=>setIdx(i=>(i-1+fotos.length)%fotos.length)} style={{position:'absolute',left:8,top:'50%',transform:'translateY(-50%)',background:'rgba(255,255,255,0.8)',border:'1px solid var(--border)',borderRadius:'50%',width:30,height:30,cursor:'pointer',fontSize:14,color:'var(--txt)',display:'flex',alignItems:'center',justifyContent:'center'}}>‹</button>
          <button onClick={()=>setIdx(i=>(i+1)%fotos.length)} style={{position:'absolute',right:8,top:'50%',transform:'translateY(-50%)',background:'rgba(255,255,255,0.8)',border:'1px solid var(--border)',borderRadius:'50%',width:30,height:30,cursor:'pointer',fontSize:14,color:'var(--txt)',display:'flex',alignItems:'center',justifyContent:'center'}}>›</button>
          <div style={{position:'absolute',bottom:8,left:'50%',transform:'translateX(-50%)',display:'flex',gap:5}}>
            {fotos.map((_,i) => (
              <button key={i} className="img-dot" onClick={()=>setIdx(i)} style={{background:i===idx?'var(--blue)':'var(--field-border)'}}/>
            ))}
          </div>
        </>
      )}
      ...
      {loading ? (
        ...
        <div style={{display:'grid',gridTemplateColumns:'300px 1fr',gap:24,alignItems:'start',animation:'fadeUp .3s ease-out'}}>
          ...
          <div style={{position:'sticky',top:80}}>
            ...
            <div style={{marginTop:16,padding:'16px',background:'white',borderRadius:14,border:'1px solid var(--border)'}}>
              <h1 style={{fontFamily:'Arial Black,sans-serif',fontWeight:900,fontSize:20,color:'var(--txt)',marginBottom:8}}>
                {meta.emoji} {tipo}
              </h1>
          ...
          <div>
            ...
            <div style={{background:'white',borderRadius:14,border:'1px solid var(--border)',padding:'14px 16px',marginBottom:14}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10,marginBottom:12}}>
                <h2 style={{fontFamily:'Arial Black,sans-serif',fontWeight:900,fontSize:15,color:'var(--txt)'}}>
                  Seleccionar marca y modelo
                </h2>
                <div style={{display:'flex',alignItems:'center',gap:8}}>
                  <span style={{fontSize:12,color:'var(--txt3)'}}>Cantidad predeterminada:</span>
                  <div style={{display:'flex',alignItems:'center'}}>
                    ...
                    <input type="number" value={qty} min={1} onChange={e=>setQty(Math.max(1,parseInt(e.target.value)||1))}
                      style={{width:56,height:28,textAlign:'center',border:'1px solid var(--field-border)',borderLeft:'none',borderRight:'none',fontSize:13,color:'var(--txt)',fontFamily:'inherit',outline:'none'}}/>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                <input placeholder="🔍 Buscar modelo..." value={search} onChange={e=>setSearch(e.target.value)}
                  style={{flex:1,minWidth:180,padding:'8px 12px',borderRadius:9,border:'1.5px solid var(--field-border)',background:'var(--field-bg)',fontSize:13,color:'var(--txt)',fontFamily:'inherit',outline:'none',transition:'border-color .18s'}}
                  onFocus={e=>e.currentTarget.style.borderColor='var(--blue)'} onBlur={e=>e.currentTarget.style.borderColor='var(--field-border)'}
                />
                <button onClick={downloadTemplate} style={{display:'flex',alignItems:'center',gap:5,padding:'8px 14px',borderRadius:9,border:'1.5px solid var(--ok)',background:'rgba(34,197,94,0.07)',color:'#16a34a',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit',transition:'all .15s',whiteSpace:'nowrap'}}>
                  ↓ Descargar plantilla
                </button>
              </div>
            </div>
              ...
              {Object.entries(filteredMarcas).map(([marca, modelos]) => (
                <div key={marca} className={`marca-row${openMarca===marca?' open':''}`}>
                  <div className="marca-header" onClick={()=>setOpenMarca(m=>m===marca?null:marca)}>
                    <span>{marca} <span style={{fontSize:12,color:'var(--txt3)',fontWeight:400}}>({Object.keys(modelos).length} modelos)</span></span>
                    <span style={{fontSize:12,color:'var(--txt3)',transition:'transform .2s',transform:openMarca===marca?'rotate(180deg)':'none'}}>▼</span>
                  </div>
                          ...
                          <div className={`modelo-item${selModel===modelo&&selMarca===marca?' sel':''}`}
                            ...
                            <div style={{display:'flex',alignItems:'center',gap:8,flex:1}}>
                              {selModel===modelo&&selMarca===marca
                                ? <span style={{color:'var(--blue)',fontWeight:900,fontSize:14}}>−</span>
                                : <span style={{color:'var(--txt3)',fontSize:14}}>+</span>
                              }
                              <span style={{fontSize:13.5,fontWeight:600,color:'var(--txt)'}}>{modelo}</span>
                            </div>
                            <button style={{padding:'4px 12px',borderRadius:100,background:selModel===modelo&&selMarca===marca?'var(--blue)':'var(--bg)',color:selModel===modelo&&selMarca===marca?'white':'var(--txt2)',border:'none',fontSize:11,fontWeight:700,cursor:'pointer',fontFamily:'inherit',transition:'all .15s'}}>
                              Todos
                            </button>
                          </div>
