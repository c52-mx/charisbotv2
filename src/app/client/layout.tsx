...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const onStorage  = () => { if (!isLocalLayoutWrite.current) setCartItems(loadCart()) }
          ...
          <div style={{position:'fixed',top:0,right:0,bottom:0,width:360,background:'white',boxShadow:'-8px 0 40px rgba(0,0,0,0.15)',zIndex:200,display:'flex',flexDirection:'column',animation:'slideIn .3s cubic-bezier(.4,0,.2,1)'}}>
                ...
                <div key={i} style={{display:'flex',alignItems:'flex-start',gap:10,padding:'12px 16px',borderBottom:'1px solid #f0f4f8'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
                    <div style={{display:'flex',alignItems:'center'}}>
                      <button onClick={()=>{ const nc=[...cartItems]; nc[i]={...nc[i],cantidad:Math.max(1,nc[i].cantidad-1)}; setCartItems(nc); saveAndNotify(nc) }} style={{width:26,height:26,border:'1px solid #d0dde8',background:'#f5f8fc',cursor:'pointer',borderRadius:'5px 0 0 5px',fontSize:13,color:'#0d2137',display:'flex',alignItems:'center',justifyContent:'center'}}>−</button>
                      ...
                      <button onClick={()=>{ const nc=[...cartItems]; nc[i]={...nc[i],cantidad:nc[i].cantidad+1}; setCartItems(nc); saveAndNotify(nc) }} style={{width:26,height:26,border:'1px solid #d0dde8',background:'#f5f8fc',cursor:'pointer',borderRadius:'0 5px 5px 0',fontSize:13,color:'#0d2137',display:'flex',alignItems:'center',justifyContent:'center'}}>+</button>
                    </div>
                    <button onClick={()=>{ const nc=cartItems.filter((_:any,j:number)=>j!==i); setCartItems(nc); saveAndNotify(nc) }} style={{background:'rgba(239,68,68,0.1)',border:'none',borderRadius:6,width:26,height:26,cursor:'pointer',color:'#ef4444',fontSize:12,display:'flex',alignItems:'center',justifyContent:'center'}}>✕</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            {cartItems.length > 0 && (
              <div style={{padding:'16px 20px',borderTop:'1px solid #e2eaf4',flexShrink:0}}>
                ...
                <button onClick={()=>{ setCartItems([]); saveAndNotify([]) }} style={{width:'100%',marginTop:8,padding:'8px',borderRadius:9,border:'1.5px solid #e2eaf4',background:'transparent',color:'#8aaac4',fontSize:12,cursor:'pointer',fontFamily:'inherit'}}>
                  Vaciar carrito
                </button>
              </div>
            )}
