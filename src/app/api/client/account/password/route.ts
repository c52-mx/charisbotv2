...
export async function POST(req: NextRequest) {
  ...
  const [user] = await query(`SELECT password_hash FROM public.usuarios WHERE id=$1`, [session.id])
  ...
  await query(`UPDATE public.usuarios SET password_hash=$1 WHERE id=$2`, [hash, session.id])
